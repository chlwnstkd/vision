...
# 웹캠으로부터 영상 가져오기(0 : 컴퓨터에 기본 설치된 웹캠 장치
# cv2.CAP_DSHOW : 화면에 웹캠영상 바로 보여주기)
cam = cv2.VideoCapture(0, cv2.CAP_DSHOW)

# 동영상은 시작부터 종료될때까지 프레임을 지속적으로 받아야하기 때문에 while문을 계속 반복함
while True:

    # 웹캠의 영상은 이미지파일과 유사한 많은 프레임들이 빠르게 움직이면서 보여줌
    # 웹갬의 영상으로부터 프레임 1개마다 읽어오기 위해 사용
    ret, webcam_image = cam.read()

    # 웹갬 영상으로부터 프레임(이미지)를 잘 받았으면 실행함
    if ret is True:
        ...
        # sigmoX : x방향 sigma로 가우시안 커널 크기(KSize)는 sigmoX 값에 따라 자동 계산
        ...
        # 스케치 효과를 주기 위해 흑백 사진과 불러 흑백 이미지를 나눈 뒤, 255를 곱함
